...
#define ZTEST(result, limit, function_val, expected_val) \
    ++result.total; \
    if (fabs(creal(function_val-expected_val))>limit*creal(expected_val) || \
        fabs(cimag(function_val-expected_val))>limit*cimag(expected_val)) {  \
        printf("failure in subtest %i: %s\n", result.total, #function_val); \
        ...
        printf("=> error %6.2g or %6.2g above limit %6.2g or %6.2g \n", \
               fabs(creal(function_val-expected_val)), \
               fabs(cimag(function_val-expected_val)), \
               limit*creal(expected_val), \
               limit*cimag(expected_val)); \
        ++result.failed; \
    }
