...
B<#ifdef __cplusplus>E<10> E<8>B<std::complexE<lt>doubleE<gt> cerfcx (std::complexE<lt>doubleE<gt> z );>E<10> E<8>B<#endif>
...
The implementation of B<erfcx> is self-contained, and improves upon the SLATEC DERFC function (or an erfcx function derived therefrom) or Cody's CALERF function (from netlib.org/specfun), while retaining near machine precision in accuracy.
